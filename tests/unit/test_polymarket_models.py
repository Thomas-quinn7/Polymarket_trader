...
class TestFakeCurrencyToDict:
    def test_required_keys(self):
        fc = FakeCurrency(balance=10000.0, deployed=500.0, pending_returns=0.0,
    ...
    def test_values(self):
        fc = FakeCurrency(balance=9500.0, deployed=500.0, pending_returns=0.0,
                          created_at=datetime(2026,1,1), updated_at=datetime(2026,1,1))
        d = fc.to_dict()
...
class TestTradeRecordToDict:
    def test_required_keys(self):
        tr = TradeRecord(
            market_id="mkt-1", market_slug="mkt", token_id="tok",
            shares=100.0, entry_price=0.985, exit_price=1.0,
            pnl=1.5, pnl_percent=1.52, edge_percent=1.5,
            status=TradeStatus.CLOSED,
            opened_at=datetime(2026,1,1), settled_at=datetime(2026,1,2),
            settlement_price=1.0,
        )
        d = tr.to_dict()
        for key in ("market_id", "market_slug", "token_id", "shares",
    ...
    def test_status_is_string(self):
        tr = TradeRecord(
            market_id="m", market_slug="m", token_id="t",
            shares=1.0, entry_price=0.5, exit_price=1.0,
            pnl=0.5, pnl_percent=100.0, edge_percent=2.0,
            status=TradeStatus.CLOSED,
...
class TestMarketCacheToDict:
    def test_required_keys(self):
        mc = MarketCache(
            ...
            yes_price=0.6, no_price=0.4, mid_price=0.5,
            cached_at=datetime(2026,1,1),
            expires_at=datetime(2026,1,1,0,5,0),
        )
        d = mc.to_dict()
        for key in ("id", "market_id", "yes_price", "no_price", "mid_price",
